...
class TheoryQuickGrowth(TheoryBase):
    ...
    def __init__(self, *args, Nmin=10, Nmax=10, deals=None, lowest=None, highest=None, capital=200000, **kwargs):
        super().__init__(*args, **kwargs)
        self.Nmin = Nmin
        self.Nmax = Nmax
    ...
    def set_stop_loss(self, status):
        ''' Устанавдливает стоплосс в зависимости от разницы между max и min
        '''
        percent_from_delta = 0.5
        delta_max_min = self.highest[ind] - self.lowest[ind]
        if status == self.STATUS['LONG']:
            self.stop_loss = self.lowest[ind] - delta_max_min * percent_from_delta
        elif status == self.STATUS['SHORT']:
            self.stop_loss = self.highest[ind] + delta_max_min * percent_from_delta
    ...
    def check(self, deals=None, highest=None, lowest=None, *args, **kwargs):
        ...
        for ind in range(1, len(self.data['<CLOSE>'])):
            ...
            if self.status == self.STATUS['NOTHING']:
                ...
                # if self.check_stop_loss(ind, self.STATUS['SHORT']):
                #     self.close_short(ind)
                #     self.deals.append(ind)
                if ind in self.highest_indexes:
                    self.close_short(ind)
                    self.open_long(ind)
                    self.set_stop_loss(ind, self.STATUS['SHORT'])
                    self.deals.append(ind)

            elif self.status == self.STATUS['LONG']:
                # if self.check_stop_loss(ind,  self.STATUS['LONG']):
                #     self.close_long(ind)
                #     self.deals.append(ind)
                if ind in self.lowest_indexes:
                    self.close_long(ind)
                    self.open_short(ind)
                    self.set_stop_loss(ind, self.STATUS['LONG'])
                    self.deals.append(ind)
